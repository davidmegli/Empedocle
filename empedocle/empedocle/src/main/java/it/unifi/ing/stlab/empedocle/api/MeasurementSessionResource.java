...
public class MeasurementSessionResource {
    ...
    public Response list(@QueryParam("offset") @DefaultValue("0") int offset,
        ...
        List<MeasurementSession> list = measurementSessionDao.find(new MeasurementSessionQueryBuilder(), offset, limit);
    ...
    public Response create(MeasurementSessionDTO dto) {
        ...
        SurveySchedule schedule = new SurveySchedule(); schedule.setId(dto.getSurveyScheduleId());
        MeasurementSessionType type = measurementSessionTypeDao.findById(dto.getTypeId());
        User author = new User(); author.setId(dto.getAuthorId());

        MeasurementSessionMapper.updateEntity(entity, dto, schedule, type, author);
